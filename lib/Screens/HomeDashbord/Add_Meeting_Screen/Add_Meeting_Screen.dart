...
class _Add_Meeting_ScreenState extends State<Add_Meeting_Screen> {
  ...
  List<Map<String, dynamic>> members = []; // List to store fetched members
  ...
  void fetchMembers() async {
    try {
      // Fetch members from the database (assuming this is asynchronous)
      List<Map<String, dynamic>> fetchedMembers =
          await DatabaseHelper.getAllMembers();

      // Update the state to reflect the fetched members
      setState(() {
          ...
          child: Column(
            children: [
              Row(
                children: [
                  Padding(
                    padding: widget.ispadding == null
                        ? const EdgeInsets.only(top: 58.0, bottom: 5, left: 5)
                        : const EdgeInsets.only(top: 58.0, bottom: 5, left: 5),
                    child: Text(
              ...
              Row(
                children: [
                  Padding(
                    padding: widget.ispadding == null
                        ? const EdgeInsets.only(top: 28.0, bottom: 5, left: 5)
                        : const EdgeInsets.only(top: 58.0, bottom: 5, left: 5),
                    child: Text(
                            ...
                            child: SizedBox(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final isAdded =
                                      addedMembersList.contains(memberName);
                                  final isInvited =
                                      InvitememberList.contains(memberName);
                                  ...
                                  return CheckboxListTile(
                                    ...
                                    onChanged: (isChecked) {
                                      setState(() {
                                        if (isChecked!) {
                                          addedMembersList.add(memberName);
                                          InvitememberList.remove(
                                              memberName); // Remove from the invite list if present
                                        } else {
                                          addedMembersList.remove(memberName);
                                        }
                                      });
                                    },
                                  );
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return CheckboxListTile(
                                    ...
                                    onChanged: (isChecked) {
                                      setState(() {
                                        if (isChecked!) {
                                          InvitememberList.add(memberName);
                                          addedMembersList.remove(
                                              memberName); // Remove from the added list if present
                                        } else {
                                          InvitememberList.remove(memberName);
                                        }
                                      });
                                    },
                                  );
                              ...
                              children: [
                                Expanded(
                                  child: SizedBox(
                                    ...
                                    child: Container(
                                      decoration:
                                          containerDecorationgrayoutline,
                                      child: TextFormField(
                                        cursorHeight: 20,
                                        controller: Copyto,
                                        decoration: const InputDecoration(
                                ...
                                OutlinedButton(
                                  style: OutlinedButton.styleFrom(
                                    side: BorderSide(
                                        color: Colors
                                            .blue), // Set the color of the outline
                                    foregroundColor: Colors
                                        .blue, // Set the color of the text
                                  ),
                                  onPressed: () {
                      ...
                      child: OutlinedButton(
                        style: OutlinedButton.styleFrom(
                          side: const BorderSide(
                              color:
                                  Colors.blue), // Set the color of the outline
                          foregroundColor:
                              Colors.blue, // Set the color of the text
                        ),
                        onPressed: () async {
                          ...
                          List<String>? memberList = addedMembersList.isNotEmpty
                              ? List.from(addedMembersList)
                              : null;
                          List<String>? inviteMemberList =
                              InvitememberList.isNotEmpty
                                  ? List.from(InvitememberList)
                                  : null;
                          List<String>? copyto = Copytolist.isNotEmpty
                              ? List.from(Copytolist)
                              : null;
                          // Insert meeting data into the database
                          String? insertedMeetingID =
                              await DatabaseHelper.insertOrUpdateMeeting(
                          ...
                          if (insertedMeetingID != null) {
                            ...
                            //     context, AppString.Meeting_save_success);
                            showDeleteConfirmationDialog(
                                context,AppString.Meeting_save_success,'Success');
                          } else {
                            showDeleteConfirmationDialog(
                                context, 'Failed to insert meeting','Alert');
                            // ScaffoldMessenger.of(context).showSnackBar(
                            //   const SnackBar(
  ...
  Future<void> showDeleteConfirmationDialog(
      BuildContext context, String text,String status) async {
